...
def httpcall(url):
    ...
    proxy = urllib.request.ProxyHandler({'http': listaproxy[index]})
...
def randomIp():
    random.seed()
    result = str(random.randint(1, 254)) + '.' + str(random.randint(1, 254)) + '.' + str(random.randint(1, 254)) + '.' + str(random.randint(1, 254))
    return result

def randomIpList():
    random.seed()
    res = ""
    for ip in range(random.randint(8, 10)):
        res = res + randomIp() + ", "
...
# Proxy
in_file = open(input("Fileproxy(.txt): "), "r")
proxyf = in_file.read()
...
# So luong
thread = int(input("So luong (3500): "))
